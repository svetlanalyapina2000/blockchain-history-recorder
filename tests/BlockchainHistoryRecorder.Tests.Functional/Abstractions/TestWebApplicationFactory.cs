...
namespace BlockchainHistoryRecorder.Tests.Functional.Abstractions;
...
public class TestWebApplicationFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
    ...
    private static readonly INetwork _network =
        new NetworkBuilder().WithCleanUp(true).WithName("blockchain_network_functional").Build();

    private readonly IContainer _cassandraContainer = new ContainerBuilder()
        .WithCleanUp(true)
        .WithImage("cassandra:latest")
        .WithName("cassandra_test_functional")
        .WithPortBinding(9044, 9042)
    ...
    private readonly IContainer _migrationContainer = new ContainerBuilder()
        .WithCleanUp(true)
        .WithImage("cassandra:latest")
        .WithName("migration_test_functional")
        .DependsOn(_network)
        .WithEnvironment("MIGRATION_DIR", "/migrations")
        .WithEnvironment("CASSANDRA_HOST", "cassandra_test_functional")
